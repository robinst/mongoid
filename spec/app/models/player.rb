class Player
  ...
  named_scope :frags_over, ->(count) {{ where: { :frags.gt => count }}}
  named_scope :deaths_under, ->(count) { criteria.where(:deaths.lt => count) }
  scope :deaths_over, ->(count) { criteria.where(:deaths.gt => count) }
